...
# =================================修改工作目录==================================
workpath = r"D:\dongshanshan"
os.chdir(workpath)
num_models = 100
for i1 in range(num_models):  # 循环创建多个模型和结果文件
    folder_path = workpath + "\\" + str(i1+1) + '/结果文件'  # 文件夹路径
    ...
    def random_quadrilateral():
        while True:
            ...
            if abs(width * height - 400) < 1:
                if height >= 10 and min(y1, y2, y3, y4) >= 0 and max(y1, y2, y3, y4) <= 100 and min(x1, x2, x3, x4) >= 40 and max(x1, x2, x3, x4) <= 190 and all(dist >= 15 for dist in [dist12, dist13, dist14, dist23, dist24]):
                # 语句块:
                    break
...
#==================================保存模型===============================
    mdb.saveAs(pathName=current_workpath + "\\结果文件" + "\\test" + str(i1+1))
